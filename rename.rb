...
def rename_files(parent_dir)
  ...
  subdirectories.each {|subdir|
    ...
    if files.length == 1
      File.rename("#{parent_dir}/#{subdir}/#{files[0]}", "#{parent_dir}/#{subdir}/#{subdir}_#{files[0]}")
      ...
      files.each_with_index {|file, index|
        ...
        File.rename("#{parent_dir}/#{subdir}/#{old_name}#{type}", "#{parent_dir}/#{subdir}/#{new_name}")
      }
      ##{subdir}_#{base}_#{index+1}#{type}"
    end
  }
end
...
def check_for_duplicates(parent_dir)
   ...
   subdirectories.each {|subdir|
     ...
     files = get_files_new_first("#{parent_dir}/#{subdir}")
    #  puts "date-sorted files: \n"
    #  files.each {|filename|
    #    file = "#{parent_dir}/#{subdir}/#{filename}"
    #    puts File.stat(file).mtime
    #  }

     files.each_with_index {|anchor, anchor_index|
       #puts "checking element #{anchor_index}"
       #puts "element is: #{anchor}"
       if anchor
      #   puts "anchoring on #{anchor}"
         anchor_size = File.stat("#{parent_dir}/#{subdir}/#{anchor}").size
         files[(anchor_index + 1)..-1].each_with_index { |candidate, candidate_index|
      #     puts "anch index: #{anchor_index}"
      #     puts "cand index: #{candidate_index}"

           if candidate
             candidate_size =
                  File.stat("#{parent_dir}/#{subdir}/#{candidate}").size
             if anchor_size == candidate_size
        #       puts "deleting file: #{candidate}"
    ...
    #  puts "date-sorted files after deletions: \n"
    #  files_new.each {|filename|
    #    file = "#{parent_dir}/#{subdir}/#{filename}"
    #    puts File.stat(file).mtime
    #  }
  ...
  # subdirectories.each {|subdir|
  #
  #   puts "looking in directory #{subdir}"
  #   files = Dir.entries("#{parent_dir}/#{subdir}")
  #   files.delete(".")
  #   files.delete("..")
  #   sizes = []
  #   files.each { |file|
  #     sizes << File.stat("#{parent_dir}/#{subdir}/#{file}").mtime
  #   }
  #   if sizes != sizes.uniq
  #     puts "duplicate files found in #{subdir}:\n "
  #     puts files
  #   end
  #
  # }
...
#  puts "old path: #{path}/#{dir}"
#  puts "new path: #{path}/#{dir}"
...
##  puts "moving the following files from #{source_dir} to #{target_dir}:\n "
#  puts "#{files} \n"
